...
const GetRout = (props) => {
                    ...
                    if (item.data().hasOwnProperty('userRoutesInfo')) {
                        if (item.data().userRoutesInfo.hasOwnProperty('routes')) {
                            item.data().userRoutesInfo.routes.forEach((item) => {
                                if ((item.parameters.count > 0) && (Date.parse(item.startDate) > new Date().getTime()) && (item.userId !== fire.auth().currentUser.uid && (item.route && item.route.waypoints[0].name.toUpperCase().includes(from.toUpperCase()))) && (item.route && item.route.waypoints[1].name.toUpperCase().includes(to.toUpperCase()))) {
    ...
    function onAcceptClick() {
        console.log(route);
        let x = fire.firestore().collection('users').doc(route.userId).get().then(res=>{
            return res.data()
        });
       console.log(x)
        ...
        fire.firestore().collection('users').doc(route.userId).get().then(result => {
            ...
            //console.log(result.userRoutesInfo.routes)
            result.userRoutesInfo.routes.forEach((item) => {
                //console.log(item)
                let ref = fire.firestore().collection('users').doc(`${route.userId}/userRoutesInfo/routes[0]`)
                //console.log(ref)
